...
class ExpenseControllerFunctionalTest {
    ...
    class GetAllExpensesTests {
        ...
        @Test
        @DisplayName("getAllExpenses_withOnlyStartDate_fallsBackToAllExpenses")
        void getAllExpenses_withOnlyStartDate_fallsBackToAllExpenses() throws Exception {
            List<ExpenseResponse> responses = List.of(sampleResponse(10L, "Groceries", "Food", LocalDate.of(2026, 6, 10)));
            when(expenseService.getAllExpenses()).thenReturn(responses);
            ...
            verify(expenseService, times(1)).getAllExpenses();
            verify(expenseService, never()).getExpensesByDateRange(any(LocalDate.class), any(LocalDate.class));
            verify(expenseService, never()).getExpensesByExpenseType(any(String.class));
        }

        @Test
        @DisplayName("getAllExpenses_withOnlyEndDate_fallsBackToAllExpenses")
        void getAllExpenses_withOnlyEndDate_fallsBackToAllExpenses() throws Exception {
            List<ExpenseResponse> responses = List.of(sampleResponse(11L, "Bus", "Transport", LocalDate.of(2026, 6, 9)));
            when(expenseService.getAllExpenses()).thenReturn(responses);

            mockMvc.perform(get("/api/expenses").param("endDate", "2026-06-30"))
                    .andExpect(status().isOk())
        ...
        @Test
        @DisplayName("getAllExpenses_withExpenseTypeAndDateRange_prioritizesExpenseType")
        void getAllExpenses_withExpenseTypeAndDateRange_prioritizesExpenseType() throws Exception {
            List<ExpenseResponse> responses = List.of(sampleResponse(12L, "Dinner", "Food", LocalDate.of(2026, 6, 8)));
            when(expenseService.getExpensesByExpenseType("Food")).thenReturn(responses);
        ...
        @Test
        @DisplayName("getAllExpenses_withInvalidDateFormat_returns400")
        void getAllExpenses_withInvalidDateFormat_returns400() throws Exception {
            mockMvc.perform(get("/api/expenses")
                            .param("startDate", "06-01-2026")
                            .param("endDate", "2026-06-30"))
                    .andExpect(status().isBadRequest());
        ...
        @Test
        @DisplayName("getAllExpenses_withEmptyExpenseType_usesExpenseTypeFilter")
        void getAllExpenses_withEmptyExpenseType_usesExpenseTypeFilter() throws Exception {
            when(expenseService.getExpensesByExpenseType("")).thenReturn(List.of());

            mockMvc.perform(get("/api/expenses").param("expenseType", ""))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$").isArray());

            verify(expenseService, times(1)).getExpensesByExpenseType("");
            verify(expenseService, never()).getAllExpenses();
            verify(expenseService, never()).getExpensesByDateRange(any(LocalDate.class), any(LocalDate.class));
        }
    }
