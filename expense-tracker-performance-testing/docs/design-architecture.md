...
| NFR | Target | Notes |
|---|---|---|
| Concurrent users | 100 users | Capacity target at any point in time |
| Active users (logged in, idle) | 50 users | Users are authenticated but not actively transacting |
| API response time | <= 3 seconds | Applies to expense API operations under the stated user-capacity targets |
