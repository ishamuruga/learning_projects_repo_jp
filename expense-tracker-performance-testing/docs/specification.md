...
| NFR | Target | Notes |
|---|---|---|
| Concurrent users | 100 users | The application must support 100 concurrent users. |
| Active users (logged in, idle) | 50 users | Users are logged in but idle, with no active action. |
| API response time | <= 3 seconds | Applies to REST API responses under the stated user-capacity targets. |
